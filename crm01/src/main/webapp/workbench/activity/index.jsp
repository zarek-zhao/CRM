...
<%--    分页插件--%>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            //以下这种做法是不行的，因为动态生成的元素是不能够以普通绑定事件的方式来进行操作的
            // $("input[name=xz]").click(function (){
            //     alert(123)
            // })

            /*
               动态生成的元素，要使用on方法的操作来触发事件
               语法：
            ...
            //为更新按钮绑定事件，执行市场活动的更新操作
            $("#updateBtn").click(function (){
                /*
                    在实际项目开发中，一定是按照先做添加，在做修改的这种顺序
                    所以，为了节省开发时间，修改操作一般都是capy添加操作
                 */
                $.ajax({
        ...
        /*
            ...
            以上为为pageList方法制定了6个入口，也就是说，在以上6个操作执行完毕后，我们必须要调用pageList方法，刷新市场活动信息列表
        ...
        function pageList(pageNo,pageSize){
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    //计算总页数
                    //parseInt(3.23) 可以将数字去掉小数位，在js中一个整数除于一个整数如果是小数不会自动转化为整数，因为js是弱类型语言
                    //因此要使用parseInt()函数进行转化，否则计算出来的总页数可能是一个浮点数，那么分页插件会出现问题
                    var totalPages = data.total%pageSize==0?data.total/pageSize:parseInt(data.total/pageSize)+1

                    //数据处理完毕后，结合分页插件查询，对前端展现分页相关的信息
...
<%--
        关于文本域textarea:
            1.一定是要以标签对的形式来表现，正常状态下标签对要紧紧的挨着
